# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'CreateCspBrandRequest' from the 'relay_rest' spec (components/schemas 'CreateCspBrandRequest').
package SignalWire::REST::Namespaces::Generated::Types::RelayRest::CreateCspBrandRequest;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::RelayRest::CreateCspBrandRequest: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'csp_self_registered' => ( is => 'ro' );
has 'name'                => ( is => 'ro' );
has 'csp_brand_reference' => ( is => 'ro' );
has 'status_callback_url' => ( is => 'ro' );

1;
