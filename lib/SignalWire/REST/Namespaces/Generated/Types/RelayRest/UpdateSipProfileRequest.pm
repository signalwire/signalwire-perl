# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'UpdateSipProfileRequest' from the 'relay_rest' spec (components/schemas 'UpdateSipProfileRequest').
package SignalWire::REST::Namespaces::Generated::Types::RelayRest::UpdateSipProfileRequest;
use strict;
use warnings;
use Moo;

# Pure data DTO: one read-only accessor per property carrying the snake
# wire key; no methods (the reference records this as a method-less type).
has 'domain_identifier' => ( is => 'ro' );
has 'default_codecs' => ( is => 'ro' );
has 'default_ciphers' => ( is => 'ro' );
has 'default_encryption' => ( is => 'ro' );
has 'default_send_as' => ( is => 'ro' );

1;
