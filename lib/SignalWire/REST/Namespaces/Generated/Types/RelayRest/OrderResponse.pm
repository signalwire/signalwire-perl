# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'OrderResponse' from the 'relay_rest' spec (components/schemas 'OrderResponse').
package SignalWire::REST::Namespaces::Generated::Types::RelayRest::OrderResponse;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::RelayRest::OrderResponse: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'id'                  => ( is => 'ro' );
has 'state'               => ( is => 'ro' );
has 'processed_at'        => ( is => 'ro' );
has 'created_at'          => ( is => 'ro' );
has 'updated_at'          => ( is => 'ro' );
has 'status_callback_url' => ( is => 'ro' );

1;
