# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'SWAIGInternalFiller' from the 'fabric' spec (components/schemas 'SWAIGInternalFiller').
package SignalWire::REST::Namespaces::Generated::Types::Fabric::SWAIGInternalFiller;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::Fabric::SWAIGInternalFiller: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'hangup'                  => ( is => 'ro' );
has 'check_time'              => ( is => 'ro' );
has 'wait_for_user'           => ( is => 'ro' );
has 'wait_seconds'            => ( is => 'ro' );
has 'adjust_response_latency' => ( is => 'ro' );
has 'next_step'               => ( is => 'ro' );
has 'change_context'          => ( is => 'ro' );
has 'get_visual_input'        => ( is => 'ro' );
has 'get_ideal_strategy'      => ( is => 'ro' );

1;
