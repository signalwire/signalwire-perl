# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'SipGateway' from the 'fabric' spec (components/schemas 'SipGateway').
package SignalWire::REST::Namespaces::Generated::Types::Fabric::SipGateway;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::Fabric::SipGateway: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'id'         => ( is => 'ro' );
has 'uri'        => ( is => 'ro' );
has 'name'       => ( is => 'ro' );
has 'ciphers'    => ( is => 'ro' );
has 'codecs'     => ( is => 'ro' );
has 'encryption' => ( is => 'ro' );

1;
