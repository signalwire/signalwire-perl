# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'SetMetaDataAction' from the 'fabric' spec (components/schemas 'SetMetaDataAction').
package SignalWire::REST::Namespaces::Generated::Types::Fabric::SetMetaDataAction;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::Fabric::SetMetaDataAction: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'set_meta_data' => ( is => 'ro' );

1;
