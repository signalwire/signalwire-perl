# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'EnterQueueObject' from the 'fabric' spec (components/schemas 'EnterQueueObject').
package SignalWire::REST::Namespaces::Generated::Types::Fabric::EnterQueueObject;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::Fabric::EnterQueueObject: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'queue_name'            => ( is => 'ro' );
has 'transfer_after_bridge' => ( is => 'ro' );
has 'status_url'            => ( is => 'ro' );
has 'wait_url'              => ( is => 'ro' );
has 'wait_time'             => ( is => 'ro' );

1;
