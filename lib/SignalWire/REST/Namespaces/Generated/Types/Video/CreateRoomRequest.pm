# Code generated by scripts/generate_rest.py; DO NOT EDIT.
#
# AUTO-GENERATED from porting-sdk/rest-apis/ (components/schemas) — regenerate with:
#   python3 scripts/generate_rest.py
#
# Generated REST wire type 'CreateRoomRequest' from the 'video' spec (components/schemas 'CreateRoomRequest').
package SignalWire::REST::Namespaces::Generated::Types::Video::CreateRoomRequest;
use strict;
use warnings;
use Moo;

# struct SignalWire::REST::Namespaces::Generated::Types::Video::CreateRoomRequest: pure data DTO (wire-response shape, not an exception).
# One read-only accessor per property carrying the snake wire key; no
# methods (the reference records this as a method-less type).
has 'name'                         => ( is => 'ro' );
has 'display_name'                 => ( is => 'ro' );
has 'description'                  => ( is => 'ro' );
has 'max_members'                  => ( is => 'ro' );
has 'quality'                      => ( is => 'ro' );
has 'join_from'                    => ( is => 'ro' );
has 'join_until'                   => ( is => 'ro' );
has 'remove_at'                    => ( is => 'ro' );
has 'remove_after_seconds_elapsed' => ( is => 'ro' );
has 'layout'                       => ( is => 'ro' );
has 'record_on_start'              => ( is => 'ro' );
has 'enable_room_previews'         => ( is => 'ro' );
has 'meta'                         => ( is => 'ro' );
has 'sync_audio_video'             => ( is => 'ro' );

1;
