...
subtest 'on_summary dispatch form invokes registered callback' => sub {
    ...
    my $ret = $a->on_summary( { stage => 'final', text => 'hello' }, { raw => 'payload' } );
    is( $ret, 'dispatched', 'returns the callback return value' );
    is_deeply( $captured[0], { stage => 'final', text => 'hello' }, 'callback got summary' );
    is_deeply( $captured[1], { raw   => 'payload' },                'callback got raw_data' );
};
...
subtest 'custom webhook URL in SWML' => sub {
    ...
    my $swml = $a->render_swml;
    my @ai   = grep { exists $_->{ai} } @{ $swml->{sections}{main} };
    like( $ai[0]{ai}{SWAIG}{functions}[0]{web_hook_url},
        qr/custom\.example\.com/, 'custom webhook used' );
};
