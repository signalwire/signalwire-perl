...
sub auth_header { return 'Basic ' . encode_base64( 'u:p', '' ) }
...
subtest 'sidecar pattern emits verb and registers tool' => sub {
    ...
    my $result = $svc->on_function_call( 'lookup_competitor', { competitor => 'ACME' }, {}, );
...
subtest 'extract_sip_username: sip URI' => sub {
    ...
    is( SignalWire::SWML::Service->extract_sip_username($body), 'alice', 'class-method form' );

    # Free-function form (single hashref arg).
    is( SignalWire::SWML::Service::extract_sip_username($body), 'alice', 'free-function form' );
};
...
subtest 'verb_registry accessor returns a registry-shaped object' => sub {
    ...
    ok( exists $reg->{handlers},
        'has "handlers" slot (Python parity: VerbHandlerRegistry._handlers)' );
