...
subtest 'sets match what join_conference accepts' => sub {
    is_deeply(SignalWire::SWAIG::JoinConference->beeps,
        [qw(true false onEnter onExit)], 'beeps set');
    is_deeply(SignalWire::SWAIG::JoinConference->records,
        ['do-not-record', 'record-from-start'], 'records set');
    is_deeply(SignalWire::SWAIG::JoinConference->trims,
        ['trim-silence', 'do-not-trim'], 'trims set');
    is_deeply(SignalWire::SWAIG::JoinConference->methods,
        [qw(GET POST)], 'methods set');
    ...
    for my $b (@{ SignalWire::SWAIG::JoinConference->beeps }) {
        ...
        ok(eval { $r->join_conference('c', beep => $b); 1 },
            "join_conference accepts beep '$b'") or diag($@);
    }
    for my $rec (@{ SignalWire::SWAIG::JoinConference->records }) {
        my $r = SignalWire::SWAIG::FunctionResult->new;
        ok(eval { $r->join_conference('c', record => $rec); 1 },
            "join_conference accepts record '$rec'") or diag($@);
    }
    for my $t (@{ SignalWire::SWAIG::JoinConference->trims }) {
        my $r = SignalWire::SWAIG::FunctionResult->new;
        ok(eval { $r->join_conference('c', trim => $t); 1 },
            "join_conference accepts trim '$t'") or diag($@);
    }
    for my $m (@{ SignalWire::SWAIG::JoinConference->methods }) {
        my $r = SignalWire::SWAIG::FunctionResult->new;
        ok(eval { $r->join_conference('c', status_callback_method => $m); 1 },
            "join_conference accepts status_callback_method '$m'") or diag($@);
        my $r2 = SignalWire::SWAIG::FunctionResult->new;
        ok(eval { $r2->join_conference('c', recording_status_callback_method => $m); 1 },
            "join_conference accepts recording_status_callback_method '$m'") or diag($@);
    }
    ...
    # module's set is exactly join_conference's accepted set.
    {
        ...
        ok(!SignalWire::SWAIG::JoinConference->is_beep('sometimes'),
    ...
    }
    {
        ...
        ok(!SignalWire::SWAIG::JoinConference->is_record('maybe'),
...
subtest 'membership predicates' => sub {
    ...
    ok(!SignalWire::SWAIG::JoinConference->is_record('true'),
        'a beep is not a record mode');

    ok(SignalWire::SWAIG::JoinConference->is_trim(DO_NOT_TRIM),
        'DO_NOT_TRIM constant is a trim');
    ok(SignalWire::SWAIG::JoinConference->is_method(POST), 'POST constant is a method');
    ok(!SignalWire::SWAIG::JoinConference->is_method('get'),
