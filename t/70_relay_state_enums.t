...
subtest 'states list the full closed sets in order' => sub {
    is_deeply(SignalWire::Relay::CallState->states,
        [qw(created ringing answered ending ended)],
        'CallState->states');
    is_deeply(SignalWire::Relay::DialState->states,
        [qw(dialing answered failed)],
        'DialState->states');
    is_deeply(SignalWire::Relay::MessageState->states,
...
subtest 'is_state membership + unknown/undef handling' => sub {
    for my $klass (qw(CallState DialState MessageState)) {
        ...
        ok(!$full->is_state('totally_made_up_state'),
            "$klass is_state(unknown) is false");
        # undef -> false, no die.
...
subtest 'is_terminal: CallState terminal = { ended }' => sub {
    ok(SignalWire::Relay::CallState->is_terminal('ended'),
        "'ended' is terminal");
    for my $st (qw(created ringing answered ending)) {
        ok(!SignalWire::Relay::CallState->is_terminal($st),
            "'$st' is NOT terminal");
    }
    ok(!SignalWire::Relay::CallState->is_terminal('made_up'),
        'unknown state is not terminal (and does not die)');
    ok(!SignalWire::Relay::CallState->is_terminal(undef),
...
subtest 'is_terminal: DialState terminal = { answered, failed }' => sub {
    ...
    ok(SignalWire::Relay::DialState->is_terminal('failed'),
        "dial 'failed' is terminal (the dial failed)");
    ok(!SignalWire::Relay::DialState->is_terminal('dialing'),
        "'dialing' is NOT terminal (in progress)");
    ok(!SignalWire::Relay::DialState->is_terminal('made_up'),
...
subtest 'is_terminal: MessageState terminal = { delivered, undelivered, failed }' => sub {
    for my $st (qw(delivered undelivered failed)) {
        ok(SignalWire::Relay::MessageState->is_terminal($st),
            "'$st' is terminal");
    }
    ...
    ok(!SignalWire::Relay::MessageState->is_terminal('made_up'),
...
subtest '3 vocabularies are distinct (answered/failed cross-checks)' => sub {
    ...
    ok(!SignalWire::Relay::CallState->is_state('failed'),
        "'failed' is NOT a CallState");
    ok(SignalWire::Relay::DialState->is_state('failed'),
        "'failed' IS a DialState");
    ok(SignalWire::Relay::MessageState->is_state('failed'),
        "'failed' IS a MessageState");
    ...
    isnt(join(',', @{ SignalWire::Relay::CallState->states }),
         join(',', @{ SignalWire::Relay::DialState->states }),
        'CallState and DialState are different sets');
    isnt(join(',', @{ SignalWire::Relay::DialState->states }),
...
subtest 'Call: typed accessor agrees with string over a real call.state flow' => sub {
    ...
    for my $step (@lifecycle) {
        ...
        is($call->is_terminal ? 1 : 0, $want_terminal,
            "is_terminal == $want_terminal at '$st'");

        # And the module predicate agrees with the instance method.
        is($call->is_terminal ? 1 : 0,
...
subtest 'Message: typed accessor agrees with string over a real messaging.state flow' => sub {
    ...
    my $sent = SignalWire::Relay::Event->parse_event('messaging.state', {
    ...
    my $delivered = SignalWire::Relay::Event->parse_event('messaging.state', {
        message_id    => 'msg-state-1',
