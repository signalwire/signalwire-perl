...
subtest 'Token generate and validate' => sub {
    my $sm = SignalWire::Security::SessionManager->new( secret_key => 'test-secret-key-12345', );
...
subtest 'Legacy methods' => sub {
    ...
    ok(
        $sm->set_session_metadata( 'call-1', 'key', 'val' ),
    ...
    for my $cid ( 'call-A', 'call-B', '' ) {
        ...
        is_deeply( $sm->get_session_metadata($cid), {}, "get_session_metadata('$cid') -> {}" );
        ok( $sm->set_session_metadata( $cid, 'k', 'v' ), "set_session_metadata('$cid', k, v) ok" );
    }
};
...
subtest 'Debug token' => sub {
    my $sm = SignalWire::Security::SessionManager->new( secret_key => 'debug-key', );
