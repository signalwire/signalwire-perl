...
sub schema { return SignalWire::SWAIG::ParameterSchema->new }
...
sub canon { my ($data) = @_; return $CANON->encode($data) }
...
subtest 'builder == hand-written across all property kinds (incl. enum)' => sub {
    ...
    my $built =
        ...
        properties => sub {
            my ($p) = @_;
            $p->string( 'city', 'City name' )->string( 'zip', 'Postal code' )->required('city');
        }
        )->required( 'service', 'date' )->to_hash;

    is_deeply( $built, $hand,
        'builder-built params are deeply identical to the hand-written hashref' );
    is( canon($built), canon($hand), 'and canonical (sorted-key) JSON is byte-for-byte identical' );

    # Pin the enum specifically: it IS the RecordCall closed set, verbatim.
    is_deeply(
        $built->{properties}{fmt}{enum},
...
subtest 'enum integrates the Tier-1 constant sets (distinct vocabularies)' => sub {
    ...
    is_deeply(
        $h->{properties}{rec_format}{enum},
        [ 'wav', 'mp3', 'mp4' ],
        'RecordCall->formats => enum [wav,mp3,mp4]'
    );
    is_deeply(
        $h->{properties}{rec_direction}{enum},
        [ 'speak', 'listen', 'both' ],
...
subtest 'required shorthand vs nested structural required' => sub {
    my $by_flag = schema()->string( 'a', 'A', required => 1 )->string( 'b', 'B' )->to_hash;
    my $by_call = schema()->string( 'a', 'A' )->string( 'b', 'B' )->required('a')->to_hash;
    ...
    is_deeply( $nested->{properties}{addr}{required},
        ['zip'], 'nested object keeps its own structural required => [zip]' );
};
...
subtest 'real define_tool with builder params: params reach SWAIG JSON + handler runs' => sub {
    ...
    is( $fn->{parameters}{properties}{service}{type},
        'string', 'service property rendered as string' );
    is(
        $fn->{parameters}{properties}{service}{description},
        'The service to book',
        'service description carried into the SWAIG JSON'
    );
    is_deeply(
        $fn->{parameters}{properties}{format}{enum},
        [ 'wav', 'mp3', 'mp4' ],
        'enum closed set rendered into the SWAIG JSON'
    );
    is( $fn->{parameters}{properties}{seats}{default},
        1, 'integer default rendered into the SWAIG JSON' );
    is_deeply( $fn->{parameters}{required},
        ['service'], 'required arrayref rendered into the SWAIG JSON' );
