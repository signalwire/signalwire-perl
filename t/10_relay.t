...
# Parse inbound call
{
    my $event = SignalWire::Relay::Event->parse_event(
        'calling.call.receive',
        {
            ...
            device     => {
                type   => 'phone',
                params => { from_number => '+15553333333', to_number => '+15554444444' }
            },
        }
    );
...
{
    my $event = SignalWire::Relay::Event->parse_event(
        'calling.call.unknown_future_thing',
        {
            foo => 'bar',
        }
    );
...
# standalone collect is StandaloneCollect (different stop verb).
{
    ...
    is( $action->_stop_method, 'calling.play_and_collect.stop', 'play_and_collect stop method' );
    ...
    is( $standalone->_stop_method, 'calling.collect.stop', 'standalone collect stop method' );

    # Play event should be ignored
    my $play_event = SignalWire::Relay::Event->parse_event(
        'calling.call.play',
        {
            control_id => 'coll-1',
            state      => 'finished',
...
# Client _handle_message -- signalwire.ping pong
{
    ...
    my @sent;
    {
        # Monkey-patching a real SDK method to capture what it was handed;
