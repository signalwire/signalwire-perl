...
subtest 'get_language_params returns undef when unset' => sub {
    ...
    is( $a->get_language_params('en-US'),
        undef, 'undef when params never set (Python returns None)' );
};

subtest 'get_language_params returns undef for unknown code' => sub {
    my $a = SignalWire::Agent::AgentBase->new( name => 'lp_get_unknown' );
    is( $a->get_language_params('zh-CN'),
        undef, 'undef for unknown code (Python returns None, no exception)' );
};
...
subtest 'set_language_params replaces existing' => sub {
    ...
    is_deeply( $a->get_language_params('en-US'), { b => 2 }, 'existing params replaced wholesale' );
};
...
subtest 'set_language_params with empty hashref removes key' => sub {
    ...
    is( $a->get_language_params('en-US'), undef, 'empty hashref clears params (Python parity)' );
...
subtest 'set_language_params on unknown code is no-op' => sub {
    ...
    is( $a->get_language_params('en-US'),
        undef, 'known language untouched when unknown code targeted' );
    ok( !exists $a->languages->[0]{params}, 'no params silently added to the wrong language' );
};
...
subtest 'native_functions in SWML' => sub {
    ...
    my $swml = $a->render_swml;
    my @ai   = grep { exists $_->{ai} } @{ $swml->{sections}{main} };
    is_deeply( $ai[0]{ai}{SWAIG}{native_functions},
        ['check_for_input'], 'native functions in SWML' );
};
