...
subtest 'record resume journals calling.record.resume' => sub {
    ...
    is( $resumes->[-1]{frame}{params}{control_id}, $action->control_id, 'control_id matches' );
    $client->disconnect;
};
...
subtest 'collect start_input_timers journals correctly' => sub {
    ...
    my $starts = RelayMockTest::journal_recv( method => 'calling.collect.start_input_timers' );
    ok( scalar @$starts, 'start_input_timers journaled' );
    is( $starts->[-1]{frame}{params}{control_id}, $action->control_id, 'control_id matches' );
    $client->disconnect;
};
...
subtest 'play volume carries negative value' => sub {
    ...
    cmp_ok( $vol->[-1]{frame}{params}{volume}, '==', -5.5, 'negative volume preserved' );
    $client->disconnect;
};
...
subtest 'unknown event type does not crash' => sub {
    my $client = _connected_client();
    RelayMockTest::push_frame( _bare_event_frame( 'nonsense.unknown', { foo => 'bar' } ) );
...
subtest 'three concurrent actions resolve independently' => sub {
    ...
    my $play1  = $call->play( play => [ { type => 'silence', params => { duration => 60 } } ], );
    my $play2  = $call->play( play => [ { type => 'silence', params => { duration => 60 } } ], );
...
subtest 'authorization state event captured' => sub {
    ...
    is( $client->authorization_state, 'test-auth-state-blob', 'authorization_state stored' );
    $client->disconnect;
};
...
subtest 'calling.error event does not crash' => sub {
    my $client = _connected_client();
    RelayMockTest::push_frame(
        _bare_event_frame( 'calling.error', { code => '5001', message => 'synthetic error' }, ) );
    _pump_until( $client, 0.5, sub { 0 } );
    ok( $client->connected, 'client still connected' );
    $client->disconnect;
};
