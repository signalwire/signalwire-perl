...
subtest 'connect returns protocol string' => sub {
    ...
    like( $client->relay_protocol, qr/^signalwire_/, 'protocol string starts with signalwire_' );
    $client->disconnect;
};
...
subtest 'connect journal records exactly one signalwire.connect frame' => sub {
    ...
    is( scalar @$entries, 1, 'exactly one signalwire.connect frame in journal' )
...
subtest 'connect journal carries contexts' => sub {
    ...
    $client->connect;
    my $entries = RelayMockTest::journal_recv( method => 'signalwire.connect' );
    is_deeply( $entries->[0]{frame}{params}{contexts}, ['default'], 'contexts list on wire' );
    $client->disconnect;
};
...
subtest 'connect journal sends event_acks=true' => sub {
    ...
    ok( $entries->[0]{frame}{params}{event_acks}, 'event_acks is truthy on wire' );
    $client->disconnect;
};
...
subtest 'reconnect with protocol string includes protocol in frame' => sub {
    ...
    ok( scalar @resume, 'resume connect frame carries the protocol' )
        ...
        . join( ',',
        map { defined $_->{frame}{params}{protocol} ? $_->{frame}{params}{protocol} : '(none)' }
            @$entries );
};
...
subtest 'connect rejects empty creds at constructor' => sub {
    ...
    like( $@, qr/project is required/i,  'connect with empty creds raises a per-variable error' );
    like( $@, qr/SIGNALWIRE_PROJECT_ID/, 'the error names the SIGNALWIRE_PROJECT_ID env var' );
};
...
subtest 'unauthenticated raw connect rejected by mock' => sub {
    ...
    my $ws = Protocol::WebSocket::Client->new( url => "ws://127.0.0.1:$RelayMockTest::WS_PORT/", );
    ...
    is( $resp->{error}{data}{signalwire_error_code},
        'AUTH_REQUIRED', 'signalwire_error_code is AUTH_REQUIRED' );
