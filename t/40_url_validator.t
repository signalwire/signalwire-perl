...
subtest 'scheme http allowed' => sub {
    ...
    ok( SignalWire::Utils::UrlValidator::validate_url('http://example.com'),
        'http allowed for public IP' );
};
...
subtest 'scheme javascript rejected' => sub {
    reset_state();
    ok( !SignalWire::Utils::UrlValidator::validate_url('javascript:alert(1)'),
        'javascript rejected' );
};
...
subtest 'IPv6 private rejected' => sub {
    ...
    ok( !SignalWire::Utils::UrlValidator::validate_url('http://ipv6-private'),
        'fc00::/7 rejected' );
};
...
subtest 'allow_private param bypasses check' => sub {
    ...
    ok( SignalWire::Utils::UrlValidator::validate_url( 'http://10.0.0.5', 1 ),
        'allow_private bypasses' );
};

subtest 'env var bypasses check' => sub {
    reset_state();

    # validate_url() reads $ENV{SWML_ALLOW_PRIVATE_URLS} from the real process
    # environment, so proving the bypass REQUIRES setting it for real. reset_state()
    # deletes the key at the top of every subtest and once more before
    # done_testing(), so the mutation never escapes the file.
...
subtest 'env var YES bypasses check' => sub {
    ...
    ## use critic
    ok( SignalWire::Utils::UrlValidator::validate_url('http://10.0.0.5'),
        'env var YES bypasses (case-insensitive)' );
};
...
subtest 'env var false does not bypass' => sub {
    ...
    ## use critic
    stub_resolver('10.0.0.5');
    ok( !SignalWire::Utils::UrlValidator::validate_url('http://internal'),
        'env var false does not bypass' );
};

subtest 'blocked networks list has all nine' => sub {
    is( scalar @SignalWire::Utils::UrlValidator::BLOCKED_NETWORKS,
        9, 'all 9 SSRF block ranges present' );
};
