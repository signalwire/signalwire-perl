...
subtest 'constants equal wire strings' => sub {
    ...
    is( DATASPHERE_SERVERLESS, 'datasphere_serverless', 'DATASPHERE_SERVERLESS constant' );
...
subtest 'all lists every built-in, matching the registry' => sub {
    ...
    is( scalar @$all, 18,      '18 built-in skill names' );
    ...
    is_deeply( $all, $registered, 'SkillName->all matches SkillRegistry->list_skills exactly' );
};
...
subtest 'is_builtin membership' => sub {
    ...
    ok( SignalWire::Skills::SkillName->is_builtin(MATH),       'MATH constant is built-in' );
    ok( !SignalWire::Skills::SkillName->is_builtin('my_custom_skill'),
        'custom skill is not built-in' );
    ok( !SignalWire::Skills::SkillName->is_builtin('datetiem'), 'typo is not built-in' );
...
subtest 'constant and string load the identical skill' => sub {
    ...
    ok( $agent_const->has_skill(DATETIME), 'has_skill(DATETIME) true — same skill key' );

    # The constant actually loaded the real skill: its tools are registered.
    ok(
        exists $agent_const->tools->{get_current_time},
    ...
    ok(
        exists $agent_str->tools->{get_current_time},
