...
subtest 'all + severity ordering' => sub {
    ...
    for my $i ( 1 .. $#sev ) {
        ok( $sev[$i] > $sev[ $i - 1 ], "severity($all->[$i]) > severity($all->[$i-1])" );
    }
    is( SignalWire::Logging::LogLevel->severity('nope'),
        undef, 'severity of unknown level is undef' );
};
...
subtest 'constant- and string-configured loggers behave identically' => sub {
    ...
    for my $threshold (@levels) {
        my $const_val =
            { debug => DEBUG, info => INFO, warn => WARN, error => ERROR }->{$threshold};
        ...
        is( $by_const->level, $by_str->level, "level attr identical for threshold '$threshold'" );
        ...
        for my $msg (@levels) {
            ...
            my $expected =
                ...
                SignalWire::Logging::LogLevel->severity($threshold) ? 1 : 0;
            is( $c, $expected,
