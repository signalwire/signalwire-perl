...
subtest 'swml_transfer' => sub {
    ...
    my $h        = result_hash($r);
    my $action   = $h->{action}[0];
    my $main_sec = $action->{SWML}{sections}{main};
    is_deeply(
        $main_sec->[0],
        { set => { ai_response => 'Transfer complete' } },
        'set ai_response'
    );
    is_deeply(
        $main_sec->[1],
        { transfer => { dest => 'https://example.com/swml' } },
        'transfer dest'
    );
...
subtest 'Speech & AI' => sub {
    ...
    $r->toggle_functions(
        [
            { function => 'f1', active => JSON::true }, { function => 'f2', active => JSON::false },
        ]
    );
    $h = result_hash($r);
...
subtest 'join_conference simple form (all defaults -> bare name)' => sub {
    ...
    is( jc_payload($r), 'lobby2', 'explicitly passing every default still collapses to bare name' );
};
...
subtest 'join_conference omits keys left at their defaults' => sub {
    ...
    for my $k (
        qw(muted beep start_on_enter end_on_exit wait_url record
...
subtest 'join_conference validations (exact Python ValueError messages)' => sub {
    ...
    # Invalid beep.
    {
        ...
        like(
            $@,
            qr/\Qbeep must be one of ['true', 'false', 'onEnter', 'onExit']\E/,
    ...
    for my $bad ( 251, 1000, 0, -1, -50 ) {
        ...
        like(
            $@,
            qr/\Qmax_participants must be a positive integer <= 250\E/,
    ...
    # Invalid record.
    {
        ...
        like(
            $@,
            qr/\Qrecord must be one of ['do-not-record', 'record-from-start']\E/,
            'record message'
        );
    }
    ...
    # Invalid trim.
    {
        ...
        like( $@, qr/\Qtrim must be one of ['trim-silence', 'do-not-trim']\E/, 'trim message' );
    }
    ...
    # Invalid status_callback_method.
    {
        ...
        like(
            $@,
            qr/\Qstatus_callback_method must be one of ['GET', 'POST']\E/,
    ...
    # Invalid recording_status_callback_method.
    {
        my $r = SignalWire::SWAIG::FunctionResult->new('conf');
        my $ok =
            eval { $r->join_conference( 'c', recording_status_callback_method => 'DELETE' ); 1 };
        ok( !$ok, 'invalid recording_status_callback_method dies' );
        like(
            $@,
            qr/\Qrecording_status_callback_method must be one of ['GET', 'POST']\E/,
...
subtest 'join_conference chaining with other actions' => sub {
    my $r =
        SignalWire::SWAIG::FunctionResult->new('multi')
    ...
    is( $h->{action}[1]{SWML}{sections}{main}[0]{join_conference}{name},
        'lobby', 'second action is the join_conference object' );
    is( $h->{action}[1]{SWML}{sections}{main}[0]{join_conference}{muted},
        JSON::true, 'muted carried into the chained join_conference' );
...
subtest 'join_room' => sub {
    ...
    $r->join_room('my-room');
    my $h        = result_hash($r);
    my $main_sec = $h->{action}[0]{SWML}{sections}{main};
    is( $main_sec->[0]{join_room}{name}, 'my-room', 'join_room' );
};
...
subtest 'sip_refer' => sub {
    ...
    my $h        = result_hash($r);
    my $main_sec = $h->{action}[0]{SWML}{sections}{main};
    is( $main_sec->[0]{sip_refer}{to_uri}, 'sip:user@example.com', 'sip_refer' );
};
...
subtest 'RPC methods' => sub {
    ...
    my $h        = result_hash($r);
    my $main_sec = $h->{action}[0]{SWML}{sections}{main};
    is( $main_sec->[0]{execute_rpc}{method},      'test_method', 'execute_rpc method' );
    is( $main_sec->[0]{execute_rpc}{params}{key}, 'value',       'execute_rpc params' );
    ...
    $h        = result_hash($r);
    $main_sec = $h->{action}[0]{SWML}{sections}{main};
    is( $main_sec->[0]{execute_rpc}{method}, 'dial', 'rpc_dial method' );
    ...
    $h        = result_hash($r);
    $main_sec = $h->{action}[0]{SWML}{sections}{main};
    is( $main_sec->[0]{execute_rpc}{method},  'ai_message', 'rpc_ai_message' );
    is( $main_sec->[0]{execute_rpc}{call_id}, 'call-123',   'rpc_ai_message call_id' );

    $r = SignalWire::SWAIG::FunctionResult->new('rpc');
    $r->rpc_ai_unhold( call_id => 'call-456' );
    $h        = result_hash($r);
    $main_sec = $h->{action}[0]{SWML}{sections}{main};
    is( $main_sec->[0]{execute_rpc}{method}, 'ai_unhold', 'rpc_ai_unhold' );
    ...
    my $unhold_rpc = $main_sec->[0]{execute_rpc};
    ...
    is_deeply(
        [ sort keys %$unhold_rpc ],
        [ sort qw(method call_id) ],
    ...
    $h        = result_hash($r);
    $main_sec = $h->{action}[0]{SWML}{sections}{main};
    ok( !exists $main_sec->[0]{execute_rpc}{params},
        'execute_rpc with empty params {} omits the params key (Python truthiness)' );
    is( $main_sec->[0]{execute_rpc}{method},
        'noop', 'execute_rpc with empty params still carries the method' );
...
subtest 'Method chaining' => sub {
    my $r =
        SignalWire::SWAIG::FunctionResult->new('chained')
        ->say('Please hold')
        ->hold(120)
...
subtest 'Payment class methods' => sub {
    my $action =
        SignalWire::SWAIG::FunctionResult->create_payment_action( 'Say', 'Enter your card' );
...
subtest 'pay always-on keys (Python parity)' => sub {
    ...
    is( pay_params($r2)->{min_postal_code_length},
        '5', 'min_postal_code_length passthrough stringified' );
...
subtest 'empty optional collections are omitted (Python truthiness)' => sub {
    ...
    my $r = SignalWire::SWAIG::FunctionResult->new;
    $r->pay(
        payment_connector_url => 'https://pay.test/c',
        parameters            => [],
        prompts               => []
    );
    my $p = pay_params($r);
    ...
    is( $p2->{prompts}[0]{for}, 'payment-card-number', 'pay keeps non-empty prompts' );
    ...
    is_deeply(
        [ sort keys %$sp ],
        [ sort qw(to_number from_number body) ],
...
subtest 'tap rtp_ptime + status_url + validation (Python parity)' => sub {
    ...
    ok( !exists tap_params($r3)->{rtp_ptime}, 'default rtp_ptime (20) omitted' );

    # rtp_ptime <= 0 dies (Python: ValueError "rtp_ptime must be a positive integer").
    my $rd = SignalWire::SWAIG::FunctionResult->new;
    ok( !eval { $rd->tap( 'wss://t.test/s', rtp_ptime => 0 ); 1 }, 'rtp_ptime => 0 dies' );
    like(
        $@,
        qr/\Qrtp_ptime must be a positive integer\E/,
        'rtp_ptime validation message matches Python'
    );
    my $rn = SignalWire::SWAIG::FunctionResult->new;
    ok( !eval { $rn->tap( 'wss://t.test/s', rtp_ptime => -5 ); 1 }, 'negative rtp_ptime dies' );
};
...
subtest 'swml_user_event' => sub {
    ...
    my $h        = result_hash($r);
    my $swml     = $h->{action}[0]{SWML};
    my $main_sec = $swml->{sections}{main};
    is( $main_sec->[0]{user_event}{event}{type},  'cards_dealt', 'user event type' );
    is( $main_sec->[0]{user_event}{event}{score}, 21,            'user event data' );
};
