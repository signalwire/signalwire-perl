...
subtest 'construction and accessors' => sub {
    ...
    is_deeply(
        $d->params,
        { to_number => '+15551234567', from_number => '+15557654321' },
        'params accessor'
    );
...
subtest 'to_hash == hand-written hashref (byte-identical)' => sub {
    ...
    for my $c (@cases) {
        is_deeply( $c->{dev}->to_hash,
            $c->{hand}, "$c->{name}: to_hash is_deeply the hand-written hashref" );
        is(
            canon( $c->{dev}->to_hash ),
            canon( $c->{hand} ),
...
subtest 'Device is an immutable snapshot of params' => sub {
    ...
    is( $emitted->{params}{to_number},
        '+15551112222', 'already-emitted to_hash is unaffected by later source mutation' );

    # Per-emission copy: mutating one emitted hash doesn't change the next.
    $emitted->{params}{to_number} = 'mutated';
    is( $d->to_hash->{params}{to_number},
        '+15551112222', 'a fresh to_hash is unaffected by mutating a prior to_hash' );
};
...
subtest 'Device drives a real Call->connect identically to a raw hashref' => sub {
    ...
    is( $p1->{devices}[0][0]{params}{to_number},
        '+15551234567', 'dispatched device param reached the RPC' );
};
