...
subtest 'add_subsection requires title' => sub {
    ...
    qr/Subsections must have a title/, 'add_subsection without title dies';
};
...
subtest 'add_section without title fails after first' => sub {
    ...
    qr/Only the first section can have no title/, 'second untitled section dies';
};
...
subtest 'add_pom_as_subsection dies when target title missing' => sub {
    ...
    qr/No section with title 'Nope' found/, 'missing title dies';
};
...
subtest 'AgentBase->pom is a fresh object each call (no caller leak)' => sub {
    ...
    is( $fresh->sections->[0]->title, 'Original', 'caller mutation of section title did not leak' );
};
