...
subtest 'tap direction is distinct from record_call direction' => sub {
    ok(SignalWire::SWAIG::Tap->is_direction('hear'),
        "tap accepts 'hear'");
    ok(!SignalWire::SWAIG::Tap->is_direction('listen'),
        "tap REJECTS 'listen' (that is record_call's word, not tap's)");
    is_deeply(SignalWire::SWAIG::Tap->directions, [qw(speak hear both)],
        'tap directions are speak/hear/both, NOT speak/listen/both');
};
...
subtest 'directions/codecs match what tap accepts' => sub {
    is_deeply(SignalWire::SWAIG::Tap->directions, [qw(speak hear both)],
        'directions lists speak, hear, both');
    is_deeply(SignalWire::SWAIG::Tap->codecs, [qw(PCMU PCMA)],
        'codecs lists PCMU, PCMA');
    ...
    # module's set is exactly tap's accepted set, not a superset.
    {
        ...
        ok(!SignalWire::SWAIG::Tap->is_direction('sideways'),
            "is_direction('sideways') false — module agrees");
    }
    {
        my $r = SignalWire::SWAIG::FunctionResult->new;
        ok(!eval { $r->tap('rtp://1.2.3.4:5000', codec => 'OPUS'); 1 },
            "tap rejects codec 'OPUS' (RELAY superset codec, outside SWAIG tap's set)");
        ok(!SignalWire::SWAIG::Tap->is_codec('OPUS'),
            "is_codec('OPUS') false — module agrees");
    }
};
...
subtest 'membership predicates' => sub {
    ...
    ok(SignalWire::SWAIG::Tap->is_direction(HEAR),
        'HEAR constant is a direction');
    ok(!SignalWire::SWAIG::Tap->is_direction('PCMU'),
        'a codec is not a direction');
    ok(!SignalWire::SWAIG::Tap->is_direction(undef), 'undef is not a direction');

    ok(SignalWire::SWAIG::Tap->is_codec('PCMU'), 'PCMU is a codec');
    ok(SignalWire::SWAIG::Tap->is_codec(PCMA),
        'PCMA constant is a codec');
    ok(!SignalWire::SWAIG::Tap->is_codec('hear'),
        'a direction is not a codec');
...
subtest 'constant and string produce the identical tap action' => sub {
    ...
    is($p_const->{direction}, 'hear',
        'constant path serialized direction => hear');
    is($p_const->{codec}, 'PCMA',
        'constant path serialized codec => PCMA');
    ...
    for my $codec (@{ SignalWire::SWAIG::Tap->codecs }) {
        my $r = SignalWire::SWAIG::FunctionResult->new;
        $r->tap('rtp://1.2.3.4:5000',
            direction => HEAR, codec => $codec_const{$codec});
        if ($codec eq 'PCMU') {
            ok(!exists tap_params($r)->{codec},
                "default codec 'PCMU' via constant is omitted (Python gating)");
        } else {
            is(tap_params($r)->{codec}, $codec,
                "constant for codec '$codec' lands as '$codec' on the wire");
        }
    }
};
...
subtest 'tap default gating: only uri emitted at defaults' => sub {
    ...
    my $p = tap_params($r);
    is_deeply([sort keys %$p], ['uri'],
        'all-defaults tap emits exactly the uri key');
