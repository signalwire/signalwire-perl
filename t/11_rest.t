...
{
    for my $h (
        '127.0.0.1', '127.0.0.1:8080', 'localhost', 'localhost:9000',
        '::1',       '[::1]',          '[::1]:7000'
        )
    {
        my $http = SignalWire::REST::HttpClient->new(
...
# ===== Fabric Namespace sub-objects =====
{
    ...
    isa_ok( $f->freeswitch_connectors,
...
# ===== Relay REST Resources =====
{
    ...
    ok(
        $client->verified_callers->can('redial_verification'),
        'verified_callers has redial_verification'
    );
    ok(
        $client->verified_callers->can('submit_verification'),
        'verified_callers has submit_verification'
    );
...
# ===== Base path verification =====
{
    ...
    is(
        $client->phone_numbers->_base_path,
        '/api/relay/rest/phone_numbers',
        'phone_numbers base path'
    );
