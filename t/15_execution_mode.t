...
# FILE-SCOPED, deliberately: get_execution_mode() / is_serverless_mode() dispatch
# on the REAL %ENV, so exercising each detection branch requires mutating the
# real process environment — a `local` copy is invisible to the function under
# test, which is the entire subject of this file. The suite owns the restore
# explicitly: %SAVED above snapshots every key at load, clear_env() runs at the
# top of each subtest, and restore_env() runs once more before done_testing(),
# so nothing escapes the file. The policy stays ON everywhere else.
...
subtest 'default is server' => sub {
    clear_env();
    is( SignalWire::Core::LoggingConfig::get_execution_mode(), 'server', 'no env vars -> server' );
};
...
subtest 'google_cloud_function via FUNCTION_TARGET' => sub {
    ...
    is( SignalWire::Core::LoggingConfig::get_execution_mode(), 'google_cloud_function' );
};
...
subtest 'azure_function via AZURE_FUNCTIONS_ENVIRONMENT' => sub {
    ...
    is( SignalWire::Core::LoggingConfig::get_execution_mode(), 'azure_function' );
};
...
subtest 'server is not serverless' => sub {
    clear_env();
    ok( !SignalWire::Utils::is_serverless_mode(), 'server mode -> not serverless' );
};
