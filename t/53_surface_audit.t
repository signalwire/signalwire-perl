...
CompileCheck::compile_ok( qq{perl -I$REPO/lib -c $SCRIPT 2>&1}, 'enumerator parses cleanly' );
...
ok( exists $modules{'signalwire.core.agent_base'}{classes}{AgentBase}, 'AgentBase class present' );
ok(
    (
        grep { $_ eq 'get_full_url' }
            @{ $modules{'signalwire.core.agent_base'}{classes}{AgentBase} }
    ),
...
ok(
    (
        grep { $_ eq 'set_prompt_text' }
            @{ $modules{'signalwire.core.mixins.prompt_mixin'}{classes}{PromptMixin} }
    ),
...
ok(
    (
        grep { $_ eq 'to_dict' }
            @{ $modules{'signalwire.core.function_result'}{classes}{FunctionResult} }
    ),
