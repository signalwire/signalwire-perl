...
subtest 'TestFabricAddresses' => sub {
    subtest 'test_list_returns_data_collection' => sub {
        ...
        ok( exists $body->{data}, "missing 'data' in body keys: " . join( ',', sort keys %$body ) );
...
subtest 'TestCxmlApplicationsCreate' => sub {
    subtest 'test_create_not_implemented' => sub {
        ...
        my $client = MockTest::client();
        ok(
            !$client->fabric->cxml_applications->can('create'),
...
subtest 'TestCallFlowsAddresses' => sub {
    subtest 'test_list_addresses_uses_singular_path' => sub {
        ...
        is( ref $body, 'HASH', 'expected hashref' );
        ok( exists $body->{data} && ref $body->{data} eq 'ARRAY', 'data is arrayref' );
...
subtest 'TestSubscribersSipEndpointOps' => sub {
    subtest 'test_get_sip_endpoint' => sub {
        my $client = MockTest::client();
        my $body   = $client->fabric->subscribers->get_sip_endpoint( 'sub-1', 'ep-1', );
        is( ref $body, 'HASH', 'expected hashref' );

        my $last = MockTest::journal_last();
        is( $last->{method}, 'GET', 'GET recorded' );
        is( $last->{path}, '/api/fabric/resources/subscribers/sub-1/sip_endpoints/ep-1',
            'path matches' );
        isnt( $last->{matched_route}, undef, 'matched_route set' );
    };
    ...
    subtest 'test_update_sip_endpoint_uses_patch' => sub {
        my $client = MockTest::client();
        my $body   = $client->fabric->subscribers->update_sip_endpoint( 'sub-1', 'ep-1',
            username => 'renamed', );
        is( ref $body, 'HASH', 'expected hashref' );
    ...
    subtest 'test_delete_sip_endpoint' => sub {
        my $client = MockTest::client();
        my $body   = $client->fabric->subscribers->delete_sip_endpoint( 'sub-1', 'ep-1', );
...
subtest 'TestFabricTokens' => sub {
    ...
    subtest 'test_refresh_subscriber_token' => sub {
        my $client = MockTest::client();
        my $body = $client->fabric->tokens->refresh_subscriber_token( refresh_token => 'abc-123', );
        is( ref $body, 'HASH', 'expected hashref' );

        my $last = MockTest::journal_last();
        is( $last->{method},   'POST',                                   'POST recorded' );
        is( $last->{path},     '/api/fabric/subscribers/tokens/refresh', 'path matches' );
        is( ref $last->{body}, 'HASH',                                   'body is hashref' );
...
subtest 'TestGenericResources' => sub {
    ...
    subtest 'test_list_addresses' => sub {
        ...
        my $last = MockTest::journal_last();
        is( $last->{method}, 'GET',                                   'GET recorded' );
        is( $last->{path},   '/api/fabric/resources/res-3/addresses', 'path matches' );
    };

    subtest 'test_assign_domain_application' => sub {
        my $client = MockTest::client();
        my $body   = $client->fabric->resources->assign_domain_application( 'res-4',
            domain_application_id => 'da-7', );
        is( ref $body, 'HASH', 'expected hashref' );

        my $last = MockTest::journal_last();
        is( $last->{method}, 'POST',                                            'POST recorded' );
        is( $last->{path},   '/api/fabric/resources/res-4/domain_applications', 'path matches' );
        is( ref $last->{body},                    'HASH', 'body is hashref' );
        is( $last->{body}{domain_application_id}, 'da-7', 'domain_application_id forwarded' );
    };
};
