...
subtest 'Step with none functions' => sub {
    ...
    $step->set_text('Read only step')->set_functions('none');
    my $h = jrt( $step->to_hash );
...
subtest 'Step behavior flags' => sub {
    ...
    $step->set_text('Flags test')->set_end(1)->set_skip_user_turn(1)->set_skip_to_next_step(1);
...
subtest 'GatherInfo' => sub {
    ...
    $step->set_text('Gather user info')->set_gather_info(
...
subtest 'GatherInfo isolated' => sub {
    ...
    $s2->set_text('Gather')->set_gather_info->add_gather_question( key => 'a', question => 'A?' );
    ...
    $s3->set_text('Gather')->set_gather_info->add_gather_question( key => 'q1', question => 'Q1?' )
