...
subtest 'response_prefix wraps success body' => sub {
    ...
    like( $out, qr/\ABEGIN-CITATION\n\n1\. Foo\b/, 'prefix prepended with blank-line separator' );
};
...
subtest 'response_postfix wraps success body' => sub {
    ...
    my $out = $skill->_wrap_response("1. Foo\n   snippet\n   http://x");
    like( $out, qr/http:\/\/x\n\nEND-CITATION\z/, 'postfix appended with blank-line separator' );
};
...
subtest 'both prefix and postfix wrap success body' => sub {
    ...
    is( $out, "P\n\nbody\n\nQ", 'both wrappers applied in canonical order' );
};
...
subtest 'no wrap when neither prefix nor postfix set' => sub {
    ...
    my $skill = $factory->new( agent => $agent, params => {} );
    my $body  = "1. Foo\n   snippet\n   http://x";
    is( $skill->_wrap_response($body),
        $body, 'response passed through unchanged when params absent' );
};
...
# called with so tests can assert per_page_timeout wiring + call counts.
{
    ...
    sub call_count { my ($self) = @_; return scalar @{ $self->{calls} } }
}

my $one_item_body =
'{"items":[{"title":"Acme Title","link":"https://example.com/a","snippet":"  Acme snippet text  "}]}';
...
subtest 'per_page_timeout falls back to bounded value when non-positive' => sub {
    ...
    is( $skill->_per_page_timeout, 15,
        'non-positive per_page_timeout -> 15s bound (never unbounded)' );
};
...
subtest 'all six latency/response params advertised in schema' => sub {
    my $schema = $factory->get_parameter_schema;
    for my $key (
        qw(response_prefix response_postfix per_page_timeout
...
subtest 'per_page_timeout sets the HTTP client timeout' => sub {
    ...
    is( $skill->_http->timeout, 4.0, 'HTTP::Tiny timeout derives from per_page_timeout' );
};
...
subtest 'overall_deadline guard returns non-empty snippet fallback' => sub {
    ...
    like( $out, qr/^Snippet-only results/, 'deadline path falls back to snippet formatting' );
    like( $out, qr/Acme snippet text/,     'deadline fallback carries the CSE snippet text' );
    unlike(
        $out,
        qr/^No results for:/,
