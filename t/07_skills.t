...
subtest 'skill construction' => sub {
    ...
    is( $skill->skill_description, 'Get current date, time, and timezone information',
        'description' );
...
subtest 'manager merges prompt sections' => sub {
    ...
    ok( $agent->prompt_has_section('Date and Time Information'),
        'datetime prompt section added to agent' );
};
...
subtest 'datasphere_serverless emits params, auth headers and foreach' => sub {
    ...
    like( $webhook->{foreach}{append},
        qr/\$\{this\.text\}/, 'foreach append interpolates this.text' );
};
