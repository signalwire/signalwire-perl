...
signalwire.ai_chat.client.AIChatClient.__init__: Perl's injectable transport is the `ua` Moo attribute (any object answering `->request`, default HTTP::Tiny) — a duck-typed handle, so it records `any`; the reference's corresponding `session` param types the concrete `aiohttp.ClientSession` class, a Python-async-library type with no Perl analogue (HTTP::Tiny is stateless, not a persistent session). Same transport-injection contract, untyped in Perl because the concrete class is Python-library-specific.
