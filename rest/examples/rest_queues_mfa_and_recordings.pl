...
if ( $recordings && $recordings->{data} && @{ $recordings->{data} } ) {
    ...
    if ( $first_rec->{id} ) {
        my $rec_detail =
            safe( 'Get recording', sub { $client->recordings->get( $first_rec->{id} ) } );
        if ($rec_detail) {
            print "  Recording: "
                . ( $rec_detail->{duration} // 'N/A' ) . "s, "
                . ( $rec_detail->{format}   // 'N/A' ) . "\n";
        }
    }
}
...
safe(
    ...
    sub {
        ...
        print "  MFA call sent: "
            . ( $voice_result->{id} // $voice_result->{request_id} // 'unknown' ) . "\n";
    }
);
    ...
    safe(
        ...
        sub {
            ...
            print "  Verification result: "
                . ( ref $verify ? 'response received' : $verify ) . "\n";
        }
    );
}
