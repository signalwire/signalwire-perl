...
The Fabric API (`/api/fabric`) manages all resource types in your SignalWire project. Most resource types support CRUD operations and address listing; a few (`addresses`, `tokens`) are special-purpose.

## Standard CRUD Pattern

The Fabric namespace exposes 16 sub-resources. The resource-typed ones share the same CRUD methods:
...
| Accessor | API Path |
|----------|----------|
| `$client->fabric->swml_scripts` | `/api/fabric/resources/swml_scripts` |
| `$client->fabric->relay_applications` | `/api/fabric/resources/relay_applications` |
| `$client->fabric->call_flows` | `/api/fabric/resources/call_flows` |
| `$client->fabric->conference_rooms` | `/api/fabric/resources/conference_rooms` |
| `$client->fabric->freeswitch_connectors` | `/api/fabric/resources/freeswitch_connectors` |
| `$client->fabric->subscribers` | `/api/fabric/resources/subscribers` |
| `$client->fabric->sip_endpoints` | `/api/fabric/resources/sip_endpoints` |
| `$client->fabric->cxml_scripts` | `/api/fabric/resources/cxml_scripts` |
| `$client->fabric->cxml_applications` | `/api/fabric/resources/cxml_applications` |
...
| Accessor | API Path |
|----------|----------|
| `$client->fabric->swml_webhooks` | `/api/fabric/resources/swml_webhooks` |
| `$client->fabric->ai_agents` | `/api/fabric/resources/ai_agents` |
| `$client->fabric->sip_gateways` | `/api/fabric/resources/sip_gateways` |
| `$client->fabric->cxml_webhooks` | `/api/fabric/resources/cxml_webhooks` |
...
```perl
# List all addresses / versions of a call flow
my $addresses = $client->fabric->call_flows->list_addresses('call-flow-uuid');
my $versions  = $client->fabric->call_flows->list_versions('call-flow-uuid');
...
my $endpoints = $client->fabric->subscribers->list_sip_endpoints('subscriber-uuid');
...
```perl
my $apps = $client->fabric->cxml_applications->list;
my $app  = $client->fabric->cxml_applications->get('app-uuid');
...
# This dies -- cXML applications cannot be created via this API:
...
my $addresses = $client->fabric->addresses->list(type => 'room');

# Get a specific address
my $address = $client->fabric->addresses->get('address-uuid');
```
...
my $refreshed = $client->fabric->tokens->refresh_subscriber_token(
