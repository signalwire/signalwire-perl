...
- ruff.toml — REPO-LINT config for this repo's 7 hand-written Python programs under scripts/ (the four code generators, the signature enumerator, the REST test generator, and _perltidy_gen.py). Tool constraint, not a placement preference: ruff discovers its config by walking UP from each linted file to the nearest ruff.toml/pyproject.toml, so a copy under eng/ would never be found for scripts/*.py and the gate would silently fall back to ruff's DEFAULT (weaker) ruleset instead of the one this repo declares. Mirrors the reference SDK's root-level [tool.ruff]. (lint-perl, 2026-07-30)
