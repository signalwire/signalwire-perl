...
sub build_ivr {
    ...
    $svc->add_verb_to_section('sales', 'play',    { url => 'say:Connecting you to sales. Please hold.' });
    $svc->add_verb_to_section('sales', 'connect', { to => '+15551234567' });

    # Support section
    $svc->add_section('support');
    $svc->add_verb_to_section('support', 'play',    { url => 'say:Connecting you to support. Please hold.' });
    $svc->add_verb_to_section('support', 'connect', { to => '+15557654321' });

    # Voicemail section
    $svc->add_section('voicemail');
    $svc->add_verb_to_section('voicemail', 'play',   { url => 'say:Please leave a message after the beep.' });
    $svc->add_verb_to_section('voicemail', 'sleep',  1000);
    $svc->add_verb_to_section('voicemail', 'record', { format => 'mp3', max_length => 120, terminators => '#' });
    $svc->add_verb_to_section('voicemail', 'play',   { url => 'say:Thank you for your message. Goodbye!' });
