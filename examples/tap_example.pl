...
my $ws_tap =
    SignalWire::SWAIG::FunctionResult->new('Starting call monitoring')
...
print "=== Basic RTP Tap ===\n";
my $rtp_tap =
    SignalWire::SWAIG::FunctionResult->new('Starting RTP monitoring')
...
my $compliance = SignalWire::SWAIG::FunctionResult->new('Setting up compliance monitoring')->tap(
...
my $cs_monitor = SignalWire::SWAIG::FunctionResult->new('Initializing quality monitoring')->tap(
    'wss://quality.company.com/cs-monitoring',
...
my $stop_recent =
    SignalWire::SWAIG::FunctionResult->new('Ending monitoring session')
    ->stop_tap()
    ->say('Call monitoring has been stopped');
print "Stop most recent tap:\n" . $json->encode( $stop_recent->to_hash ) . "\n";

my $stop_specific =
    SignalWire::SWAIG::FunctionResult->new('Ending compliance monitoring')
...
my $start = SignalWire::SWAIG::FunctionResult->new('Call center session starting')
    ->tap(
    'wss://callcenter.company.com/agent-monitoring',
...
my $end =
    SignalWire::SWAIG::FunctionResult->new('Ending call session')
