...
my $command_processor =
    SignalWire::DataMap->new('command_processor')
...
my $advanced_api =
    SignalWire::DataMap->new('advanced_api_tool')
    ...
    ->output( SignalWire::SWAIG::FunctionResult->new('API result: ${response.data}') )
    ->webhook(
    'GET',
    'https://backup-api.example.com/simple',
    headers => { Accept => 'application/json' },
    )
    ->output( SignalWire::SWAIG::FunctionResult->new('Backup result: ${response.data}') )
    ->fallback_output(
...
my $form_tool =
    SignalWire::DataMap->new('form_submission_tool')
    ...
    ->output(
    SignalWire::SWAIG::FunctionResult->new('Form submitted successfully for ${args.name}') )
    ->error_keys( [ 'error', 'validation_errors' ] );

# 4. Conditional logic with expressions and fallback
my $calculator =
    SignalWire::DataMap->new('smart_calculator')
