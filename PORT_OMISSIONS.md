...
signalwire.core.web.HostAppRouter: impossible: behaviour-neutral named cross-port type for as_router()'s "embed my routes in a host app" unit; Perl's routable unit is a bare PSGI coderef (sub { my $env=shift; [$status,$headers,$body] }) mountable via Plack::Builder, so there is no Perl CLASS to enumerate — the capability is present via as_router() (reconciled to class:signalwire.core.web.HostAppRouter at the signature layer through the perl CodeRef→HostAppRouter type-alias), and the method-less named-type CLASS has no port-side surface analog (TS's Hono / Ruby's Rack::Builder likewise have no standalone HostAppRouter class)
...
signalwire.skills.native_vector_search.skill.NativeVectorSearchSkill: approved: Python-only RAG/vector-search subsystem (native_vector_search skill), not ported to any SDK — user, 2026-07 pass (§I.1)
...
signalwire.skills.native_vector_search.skill.NativeVectorSearchSkill.get_hints: approved: Python-only RAG/vector-search subsystem (native_vector_search skill), not ported to any SDK — user, 2026-07 pass (§I.1)
signalwire.skills.native_vector_search.skill.NativeVectorSearchSkill.get_instance_key: approved: Python-only RAG/vector-search subsystem (native_vector_search skill), not ported to any SDK — user, 2026-07 pass (§I.1)
signalwire.skills.native_vector_search.skill.NativeVectorSearchSkill.get_parameter_schema: approved: Python-only RAG/vector-search subsystem (native_vector_search skill), not ported to any SDK — user, 2026-07 pass (§I.1)
...
signalwire.skills.native_vector_search.skill.NativeVectorSearchSkill.register_tools: approved: Python-only RAG/vector-search subsystem (native_vector_search skill), not ported to any SDK — user, 2026-07 pass (§I.1)
signalwire.skills.native_vector_search.skill.NativeVectorSearchSkill.setup: approved: Python-only RAG/vector-search subsystem (native_vector_search skill), not ported to any SDK — user, 2026-07 pass (§I.1)
