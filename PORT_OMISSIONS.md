...
signalwire.ai_chat.client.AIChatClient.close: impossible: closes the persistent aiohttp.ClientSession the async client holds open; Perl's HTTP::Tiny transport is stateless (a fresh connection per request, nothing to close) — the resource-cleanup companion of the __aenter__/__aexit__ protocol, so no Perl analog (TS/PHP omit identically)
